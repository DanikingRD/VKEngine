...
const char* vulkan_result_to_str(VkResult result, bool extended) {
    ...
    case VK_PIPELINE_COMPILE_REQUIRED_EXT:
        return !extended ? "VK_PIPELINE_COMPILE_REQUIRED_EXT"
                         : "VK_PIPELINE_COMPILE_REQUIRED_EXT A requested pipeline creation "
    ...
    case VK_ERROR_FRAGMENTED_POOL:
        return !extended
                   ...
                   : "VK_ERROR_FRAGMENTED_POOL A pool allocation has failed due to fragmentation "
                     ...
                     "host or device memory was made to accommodate the new allocation. This "
                     "should be returned in preference to VK_ERROR_OUT_OF_POOL_MEMORY, but only if "
                     "the implementation is certain that the pool allocation failure was due to "
                     "fragmentation.";
    ...
    // because the requested address is not available. A shader group handle assignment failed
    ...
    case VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT:
        return !extended
                   ...
                   : "VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT An operation on a swapchain "
                     "created with VK_FULL_SCREEN_EXCLUSIVE_APPLICATION_CONTROLLED_EXT failed as "
                     "it did not have exlusive full-screen access. This may occur due to "
